def two_sum_brute_force(nums, target):
    # O primeiro loop percorre cada número da lista
    for i in range(len(nums)):
        # O segundo loop percorre os números seguintes ao índice 'i'
        for j in range(i + 1, len(nums)):
            # Verifica se a soma dos valores nos índices i e j é igual ao alvo
            if nums[i] + nums[j] == target:
                return [i, j]
    
    # Caso percorra toda a lista e não encontre nada, retorna uma lista vazia
    return []
